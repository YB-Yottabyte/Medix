...
const DEMO_TIMESTAMP = /(\[\d+:\d{2}(?:[–-]\d+:\d{2})?\])/g;

/**
 * Mirrors the real chat: citations render as monospace chips rather than raw
 * bracket text, so the mockup matches the product it is advertising.
 */
function renderDemoAnswer(text: string) {
  return text.split(DEMO_TIMESTAMP).map((chunk, index) =>
    DEMO_TIMESTAMP.test(chunk) ? (
      <span
        className="mx-0.5 inline-block rounded-md bg-sky-300/15 px-1.5 py-0.5 font-mono font-semibold text-[0.8em] text-sky-200/90"
        // biome-ignore lint/suspicious/noArrayIndexKey: static demo string
        key={index}
      >
        {chunk.replace(/[[\]]/g, "")}
      </span>
                    ...
                    <div className="space-y-5">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1 rounded-[1.5rem] border border-white/10 bg-white/6 px-5 py-4 shadow-sm">
                          ...
                          <div className="whitespace-pre-line text-left text-[15px] leading-7 text-white/84">
                            {typedResponse ? (
                              renderDemoAnswer(typedResponse)
                            ) : (
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="rounded-[1.2rem] border border-white/8 bg-white/5 px-4 py-4 shadow-sm">
                      ...
                      <div className="mt-2 text-sm leading-6 text-white/68">
                        Ask a question, wait through a brief thinking state, and
                        watch Medix attach grounded procedural guidance for the
                        caregiver.
                      </div>
                    </div>
