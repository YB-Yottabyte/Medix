  ...
  try {
    ...
    if (clerkUser) {
      const [updatedUser] = await db
        ...
        .set({
          email,
          name: name ?? clerkUser.name,
          image: image ?? clerkUser.image,
          emailVerified: true,
          updatedAt: new Date(),
        })
    ...
    // Link an existing Medix account by verified email so adopting Clerk does
    // not detach the user's saved chats from the internal UUID owner.
    ...
    throw new ChatbotError(
      "bad_request:database",
      "Failed to connect Clerk user to Medix"
    );
  }
}
...
export async function deleteUserAccount({ userId }: { userId: string }) {
  try {
    return await db.transaction(async (transaction) => {
      const userChats = await transaction
        .select({ id: chat.id })
        .from(chat)
        .where(eq(chat.userId, userId));
      ...
      if (chatIds.length > 0) {
        await transaction.delete(vote).where(inArray(vote.chatId, chatIds));
        await transaction
          .delete(message)
