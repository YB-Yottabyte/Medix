...
class TranscriptFetcher:
    ...
    def fetch_snippets(
        ...
        """
        Fetch raw transcript snippets for a video, optionally filtered to a time range.
        Returns a list like:
        [{"start": 12.3, "duration": 4.1, "text": "..."}, ...]
        """
        if not self.available or not video_id:
            return []
        ...
        if cached is None:
            try:
                ...
                self._save_cache(video_id, cached)
            except Exception:
                return []

        return self._filter_snippets(cached, start, end)
    ...
    def _filter_and_format(
        ...
        filtered = self._filter_snippets(snippets, start, end)
