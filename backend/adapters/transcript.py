"""Cached YouTube transcript adapter."""
...
class TranscriptFetcher:
    def __init__(self, cache_dir: str | Path = "./data/transcript_cache"):
    ...
    def fetch(
        ...
        snippets = self.fetch_cues(video_id)
        return self._filter_and_format(snippets, start, end) if snippets is not None else None

    def fetch_cues(self, video_id: str) -> list[dict] | None:
        """Return timestamped cues, populating the local cache when necessary."""
        if not self.available or not video_id:
            return None
    ...
    def _save_cache(self, video_id: str, snippets: list):
        try:
            with self._cache_path(video_id).open("w", encoding="utf-8") as f:
