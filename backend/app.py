...
CACHE_DIR = PROJECT_ROOT / "data" / "cache_medvidqa_verified"
...
if CACHE_DIR.exists() and (CACHE_DIR / "procedures.pkl").exists():
    db.load(CACHE_DIR)
else:
    sys.exit(1)
...
def voice_transcribe():
    ...
    try:
        ...
        return jsonify({"success": True, "text": text, "method": "whisper-large-v3"})
...
def multimodal_query():
    ...
    try:
        ...
        if visual_context and visual_context.get("success"):
            ...
            retrieved = retriever.search_with_context(
                enriched_query,
                body_part=body_part,
                condition=condition,
            )
        ...
        if visual_context and visual_context.get("success"):
            visual_context_block = (
                ...
                f"- Description: {visual_context.get('description', '')}\n"
