...
Uses Llama 4 Scout (Vision Language Model) via Groq API to describe
medical images, then searches the procedures database with the description.

Pipeline:
  1. Send image to Llama 4 Scout → get detailed medical description
...
class MedicalImageRecognizer:
    """
    VLM-based medical image recognition using Llama 4 Scout via Groq.
    Much more accurate than CLIP for medical diagnosis.
    """
    ...
    def __init__(self, config, database):
        ...
        api_key = config.get("ai", {}).get("groq", {}).get("api_key", "")
        if not api_key:
            raise ValueError("Groq API key not found in config")

        self.client = Groq(api_key=api_key)
        print(f"Vision model: {self.VISION_MODEL} (via Groq)")
        print("Image recognition ready (Llama 4 Scout VLM)")
    ...
    def _analyze_image(self, image):
        """Send image to Llama 4 Scout and get structured analysis."""
    ...
    def _build_action_query(body_part, condition):
        ...
        elif any(kw in cond for kw in ["sprain", "strain"]):
            return f"How to tape or wrap a sprained {bp}"
        elif any(kw in cond for kw in ["wound", "cut", "laceration"]):
            return f"How to bandage a {bp} wound"
        elif any(kw in cond for kw in ["burn"]):
            return f"First aid for {bp} burn"
        elif any(kw in cond for kw in ["bleeding"]):
            return f"How to stop bleeding on the {bp}"
        elif any(kw in cond for kw in ["dislocation", "dislocated"]):
            return f"First aid for dislocated {bp}"
        elif any(kw in cond for kw in ["cardiac", "heart attack", "cpr"]):
            return "How to perform CPR cardiac arrest"
        elif any(kw in cond for kw in ["choking"]):
    ...
    def recognize_from_image(self, image):
        """
        Main entry point.
        1. Llama 4 Scout analyses the image → structured medical description
        2. Use the description as a search query → find matching procedures
        3. Return ranked results
        """
        print("\n  Analyzing image with Llama 4 Scout VLM...")
        ...
        is_emergency = severity == "emergency" or any(
            ...
            for kw in ["cardiac", "heart attack", "choking", "unconscious",
                       "cpr", "not breathing", "severe bleeding"]
        )
        ...
        for i, proc in enumerate(self.database.procedures):
            ...
            if any(kw in cond_lower for kw in ["fracture", "broken", "break", "crack"]):
                if any(kw in q for kw in ["fracture", "splint", "broken", "break", "first aid", "bandage"]):
                    similarities[i] *= 2.0   # strong boost for fracture treatment
                elif any(kw in q for kw in ["exercise", "stretch", "flex", "massage", "strengthen", "yoga", "workout"]):
            ...
            # If VLM detected a WOUND / CUT / BLEEDING
                ...
                if any(kw in q for kw in ["wound", "cut", "bandage", "bleeding", "first aid", "stitch"]):
                    similarities[i] *= 1.8
        ...
        detected_group = None
        for group_name, words in body_groups.items():
        ...
        return {
            ...
            "all_matches": results,
            "recognition_method": "Llama-4-Scout VLM",
